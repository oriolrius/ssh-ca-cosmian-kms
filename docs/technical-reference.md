...
Architecture, Cryptographic Internals, Configuration & Automation

*A Comprehensive Technical Reference with Sequence Diagrams*

April 2026

**Primary Sources**

OpenSSH PROTOCOL.certkeys • ssh-keygen(1) • sshd\_config(5)
...
community.crypto.openssh\_cert / openssh\_keypair Ansible modules
...
![](media/586b6632e399e3ddb431c512d989ea6e43bb9bc2.png){width="6.041666666666667in" height="5.520833333333333in"}
...
![](media/12ba1c372e07e979a227ae136a59e2d3eac71cb8.png){width="6.041666666666667in" height="4.583333333333333in"}
...
>
> scp root\@server:/etc/ssh/ssh\_host\_ed25519\_key.pub ./
>
...
>
> \# Result: ssh\_host\_ed25519\_key-cert.pub
>
...
>
> HostCertificate /etc/ssh/ssh\_host\_ed25519\_key-cert.pub
>
...
>
> \# /etc/ssh/ssh\_known\_hosts or via ssh\_config GlobalKnownHostsFile
...
![](media/e826eb343e6f75995ac364ba2875e704465b9eb5.png){width="6.25in" height="5.104166666666667in"}
...
> \# /etc/ssh/sshd\_config
>
> AuthorizedPrincipalsFile /etc/ssh/auth\_principals/%u
>
> \# /etc/ssh/auth\_principals/root (who can login as root?)
>
> admin
>
> root-everywhere
>
> \# /etc/ssh/auth\_principals/deploy (who can login as deploy?)
>
> deployer
>
> zone-webservers
...
>
> CertificateFile \~/.ssh/id\_ed25519-cert.pub
>
> IdentitiesOnly yes
...
>
> \# sshd\_config: RevokedKeys /etc/ssh/revoked
>
...
![](media/b25a3bfe615d49e7ca0ea7c01d38b1e42207ec3d.png){width="6.041666666666667in" height="5.520833333333333in"}
...
> \- hosts: d13
>
> gather\_facts: yes \# Need ansible\_fqdn, ansible\_all\_ipv4\_addresses
>
> remote\_user: jp
>
> vars:
...
>
> community.crypto.openssh\_keypair:
>
> backend: \"opensshbin\"
>
> path: \"{{ dirname }}/ssh\_host\_ed25519\_key\"
>
> comment: \"ansible-made™ for host {{ inventory\_hostname }}\"
>
> type: \"ed25519\"
>
> become: true
...
>
> \# Sourced from keydata.public\_key (returned metadata)
>
> \- name: Save public host key to local temporary file
>
> ansible.builtin.copy:
>
> content: \"{{ keydata.public\_key }}\"
>
...
>
> \# use\_agent: true = CA passphrase from ssh-agent, never on disk
>
> \- name: Sign SSH certificate on local copy of public host key
>
> community.crypto.openssh\_cert:
>
> identifier: \"{{ inventory\_hostname }}\"
>
> public\_key: \"{{ p.path }}/{{ keydata.filename \| basename }}\"
>
> principals:
>
> \"{{ \[ ansible\_fqdn \] + ansible\_all\_ipv4\_addresses }}\"
>
> path: \"{{ p.path }}/{{ keydata.filename \| basename }}-cert.pub\"
>
> serial\_number: 10
>
> signing\_key: \"../CA/ssh-ca\"
>
> use\_agent: true
>
> type: \"host\"
...
>
> \"filename\": \"/etc/ssh/ssh\_host\_ed25519\_key\",
>
> \"fingerprint\": \"SHA256:3zm2UIy\...2Y\",
>
> \"public\_key\": \"ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAA\...\",
>
> \"size\": 256,
>
> \"type\": \"ed25519\"
...
>
> \[kms\_config.http\_config\]
>
> server\_url = \"https://kms.example.com:9998\"
>
> ssl\_client\_pkcs12\_path = \"./certificates/controller.p12\"
>
> ssl\_client\_pkcs12\_password = \"p12\_password\"
>
> \# Register the module with p11-kit:
>
> sudo tee /etc/pkcs11/modules/cosmian\_pkcs11.module \<\<EOF
>
> module: /usr/local/lib/libcosmian\_pkcs11.so
>
> EOF
...
>
> ssh-add -s /usr/local/lib/libcosmian\_pkcs11.so
>
...
>
> \# community.crypto.openssh\_cert with use\_agent: true
>
...
>
> ssh-keygen -D /usr/local/lib/libcosmian\_pkcs11.so \\
>
> -s CA/ssh-ca.pub \\
>
